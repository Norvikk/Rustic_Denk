...
pub fn run(burst: bool, size_key: i64) -> anyhow::Result<String> {
    ...
    if burst {
        ...
        match retrieve_path(("bricked", "keys"), ".dnk", true) {
            Ok(value) => {
        ...
        match retrieve_path(("bricked", "keys"), ".dnk", false) {
            Ok(value) => {
                bricked_path = value.0;
                keys_path = value.1
            }
...
fn retrieve_path(name: (&str, &str), format: &str, burst: bool) -> anyhow::Result<(String, String)> {
    let path;
    if burst {
